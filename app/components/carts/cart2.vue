    ...
    <div class="max-w-screen-xl max-lg:max-w-xl mx-auto">
      <div class="grid lg:grid-cols-2 gap-12">
            ...
            <div v-for="item in cart.items" :key="item.id" class="space-y-4">
              <div class="flex gap-4 max-sm:flex-col">
                <div class="w-24 h-24 shrink-0 bg-purple-50 p-2 rounded-md">
                  <img :src="item.product?.imageUrl || 'https://readymadeui.com/images/product14.webp'"
                ...
                <div class="w-full flex justify-between gap-4">
                  ...
                  <div class="flex flex-col justify-between items-end gap-4">
                    <div>
                      <svg @click="$emit('remove', item.id)" xmlns="http://www.w3.org/2000/svg"
                        class="w-4 fill-red-500 cursor-pointer" viewBox="0 0 24 24">
                        <path
                    ...
                    <div
                      class="flex items-center px-2.5 py-1.5 border border-gray-400 text-slate-900 text-xs font-medium rounded-md">
                      <button @click="$emit('decrementItem', item.id)" type="button">
                      ...
                      <button @click="$emit('incrementItem', item.id)" type="button">
        ...
        <div class="max-w-4xl w-full h-max rounded-md">
          <!-- GLOBAL ALERT -->
          <div v-if="alert.show"
            class="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-xl rounded-xl shadow-xl text-white p-4 text-center text-lg font-semibold"
            :class="alert.type === 'success' ? 'bg-green-600' : 'bg-red-600'">
            {{ alert.message }}
          </div>
          ...
          <form @submit.prevent="checkout">
            <div>
              ...
              <div class="grid lg:grid-cols-2 gap-y-6 gap-x-4">
                ...
                <div>
                  <label class="text-sm font-medium mb-2">Phone No.</label>
                  <input v-model="form.phone" type="number" class="input-field" />
                </div>

                <div>
                  <label class="text-sm font-medium mb-2">Address Line</label>
                  <input v-model="form.address" class="input-field" />
                </div>

                <div>
                  <label class="text-sm font-medium mb-2">City</label>
                  <input v-model="form.city" class="input-field" />
                </div>

                <div>
                  <label class="text-sm font-medium mb-2">State</label>
                  <input v-model="form.state" class="input-field" />
                </div>

                <div>
                  <label class="text-sm font-medium mb-2">Zip Code</label>
                  <input v-model="form.zip" class="input-field" />
                </div>
              </div>
            </div>
            ...
            <div class="mt-10">
              ...
              <div class="flex gap-4 max-lg:flex-col mt-8">
                <button type="button" class="btn-gray" @click="$emit('continueShopping')">
                  Continue Shopping
                </button>
...
const checkout = async () => {
  try {
    ...
    if (res.statusCode !== "00") {
      showAlert(res.message || "Checkout failed", "error");
      return;
    }

    showAlert(res.message || "Checkout successful", "success");
